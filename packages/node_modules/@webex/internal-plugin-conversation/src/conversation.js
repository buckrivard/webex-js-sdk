...
import {
  ...
  isDeleteActivity,
  getPublishedDate, isNewer, sortActivitiesByPublishedDate,
  getParentId,
...
const Conversation = WebexPlugin.extend({
  ...
  */
   ...
   * @param {object} [query] object containing query string values to be appended to the url
   * @param {string} [query.sinceDate] The date to get the parents from.
   * @param {string} [query.maxDate] The parents published date is before this date
   * @param {Number} [query.includeParentCount] max number of parents to return.
   * @param {string} [query.activityType] The type of children to return the parents of, a null value here returns parents of all types of children.
   ...
   * @param {object} query object containing query string values to be appended to the url
   * @param {string} query.activityType The type of child activities to return.
   * The value is one of 'reply', 'edit', 'cardAction', 'reaction', 'reactionSummary', 'reactionSelfSummary'
   * @param {number} [query.limit] The limit of child activities that can be returned per request
   * @param {boolean} [query.latestActivityFirst] Sort order for the child activities
   * @param {boolean} [query.includeParentActivity] Enables the parent activity to be returned in the activity list
   ...
   */
  async listChildActivitiesByParentId(conversationUrl, activityParentId, query) {
    const params = {
      method: 'GET',
  ...
  listActivitiesThreadOrdered(options) {
    ...
    // TODO: not convinced we need this -- also requires _listActivitiesThreadOrdered to have a finally block to do anything useful
    const cleanup = () => {
      if (threadOrderer) {
        threadOrderer.return();
      }
    };
  ...
  // TODO: properly JSDoc my types? Does this even work?
  /**
  ...
  async* _listActivitiesThreadOrdered(options = {}) {
    ...
    // store all children to ensure no errant queries miss activities
    ...
    // query resolver. closes over oldestAct, newestAct, and batchSize to build query
    const getQuery = (type, activityToSearch = {}) => {
      switch (type) {
        case NEWER: {
          const sinceDate = getPublishedDate(getNewestAct()) + 1;
        ...
        case OLDER: {
          const maxDate = getPublishedDate(getOldestAct()) - 1;
    ...
    // set initial query
    let query = getQuery(queryType, search);

    /* eslint-disable no-await-in-loop */
    /* eslint-disable no-loop-func */

    // TODO: consider ES6 class for noMoreActs, noOlder, noNewer
    // NOT ES6, use revealing module pattern
    while (true) {
      // ***********************************************
      ...
      // ***********************************************
      // HELPERS
      // mostly used to close over state variables and to help self-document
      // ***********************************************
      ...
      const handleNewActivity = (activity) => {
        ...
        const activityHandler = activityHandlerByType[actType];
      ...
      while (!getNoMoreActs()) {
        // break out of infinite loop, if necesseary
        fetchLoopCount += 1;
        ...
        // default parents to empty object so we avoid recursive children fetches when possible
        const [
          allBatchActivities,
          parents = {}
        ...
        */
        ...
        if (replyIds.length || editIds.length || reactionIds.length) {
          ...
          // dont forget the new reply reactions!
          // introduces waterfall, but im not sure it can be avoided...
        ...
        for (const rootActivity of rootActivityHash.values()) {
          ...
          const repliesByRootId = getValue(replyActivityHash, rootId);
        ...
        if (queryType === INITIAL) {
          query = getQuery(OLDER);
        }
        else {
          query = getQuery(queryType, search);
        }
      ...
      const getRepliesByParentId = (replyParentId) => {
        ...
        const repliesByParentId = getValue(replyActivityHash, replyParentId);
        ...
        sortedReplies.forEach((replyActivity) => {
          ...
          const edit = getValue(editActivityHash, replyId);
          const reaction = getValue(reactionActivityHash, replyId);
          const reactionSelf = getValue(reactionSelfActivityHash, replyId);
      ...
      const orderedRoots = sortActivitiesByPublishedDate(getActivityObjectsFromMap(rootActivityHash));

      orderedRoots.forEach((rootActivity) => {
        const rootId = rootActivity.id;
        const replies = getRepliesByParentId(rootId);
        const edit = getValue(editActivityHash, rootId);
        const reaction = getValue(reactionActivityHash, rootId);
        const reactionSelf = getValue(reactionSelfActivityHash, rootId);
